 ...
 */
function assertDifferent<T = unknown, U = unknown>(a: T, b: U): void;
/**
 * Compares two values and throws an error if they are equal.
 *
 * @template T The type of the first value being compared.
 ...
 * @throws A `TypeError` if the values `a` and `b` are equal.
 */
function assertDifferent<T = unknown, U = unknown>(
	a: T,
	b: U,
	name1: string
): void;
...
function assertDifferent<T = unknown, U = unknown>(
	...
	if (comparator(a, b))
		throw new TypeError(
			`"${name1}" must NOT be equal to "${name2}". Got "${a}" (of type "${typeof a}") and "${b}" (of type "${typeof b}").`
		);
}
