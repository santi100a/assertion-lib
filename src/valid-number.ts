 ...
 */
function assertValidNumber(num: number): void;
/**
 ...
 */
function assertValidNumber(num: number, name: string): void;

function assertValidNumber(num: number, name = 'num') {
    assertTypeOf(num, 'number', name);
    if (isNaN(num))
        throw new TypeError(`"${name}" must not be NaN. Got ${num}.`);
}
